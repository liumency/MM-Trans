...
def ResNet34(output_stride, BatchNorm=nn.BatchNorm2d, pretrained=True, in_c=3, f_c = 64):
    model = ResNet(BasicBlock, [3, 4, 6, 3], output_stride, BatchNorm, in_c=in_c, f_c=f_c)
...
class ResNet(nn.Module):
    ...
    def __init__(self,  block, layers, output_stride, BatchNorm, in_c=3, f_c = 64):
        ...
        self.conv = nn.Conv2d(960, 256, kernel_size=1, padding=0)
        self.dr2 = DR(64, 96)
        self.dr3 = DR(128, 96)
        self.dr4 = DR(256, 96)
        self.dr5 = DR(512, 96)
        self.last_conv = nn.Sequential(nn.Conv2d(384, 256, kernel_size=3, stride=1, padding=1, bias=False),
                                       ...
                                       nn.ReLU(),
                                       nn.Dropout(0.5),
                                       nn.Conv2d(256, self.f_c, kernel_size=1, stride=1, padding=0, bias=False),
                                       BatchNorm(self.f_c),
                                       nn.ReLU(),
                                       )
